...
from src.croco.models.pos_embed import get_1d_sincos_pos_embed_from_grid
...
class FlashDUSt3R(
    ...
    def __init__(
        ...
        **croco_kwargs,
        ...
        self.register_buffer(
            "image_idx_emb",
            torch.from_numpy(
                get_1d_sincos_pos_embed_from_grid(self.dec_embed_dim, np.arange(10000))
            ).float(),
            persistent=False,
        )
    ...
    def _decoder(self, encoded_feats, positions, image_ids):
        ...
        if self.random_image_idx_embedding:
            # Use random image IDs if the flag is set
            unique_image_ids = set()  # Track unique random image IDs
            image_pos = []

            for i in range(len(encoded_feats)):
                if i == 0:
                    # The first view always gets image_id=0
                    image_id = 0
                else:
                    # Randomly pick a non-repeating image ID between 1 and 9999
                    while True:
                        image_id = torch.randint(1, self.image_idx_emb.shape[0], (1,)).item()
                        if image_id not in unique_image_ids:
                            unique_image_ids.add(image_id)
                            break

                num_patches = encoded_feats[i].shape[1]
                image_pos.append(self.image_idx_emb[image_id].unsqueeze(0).repeat(num_patches, 1))

            # Concatenate all positional embeddings for image IDs
            image_pos = torch.cat(image_pos, dim=0)
        else:
